...
class PropertyForm(forms.ModelForm):
    photos = MultiFileField(
        required=False,
        widget=MultiFileInput(attrs={"class": "block w-full text-sm text-gray-300 file:bg-gold file:text-[#232323] file:font-semibold file:px-4 file:py-2 file:rounded file:border-0 file:mr-2", "accept": "image/*,video/*"}),
        ...
        if commit:
            ...
            for f in files:
                if f.content_type.startswith('image/'):
                    ...
                    Video.objects.create(property=property_obj, video=compressed_video, order=video_order)
                    video_order += 1
        return property_obj
