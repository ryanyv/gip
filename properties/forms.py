...
class PropertyForm(forms.ModelForm):
    photos = MultiFileField(
        ...
        help_text="Upload photos or videos",
    )
    ...
    def save(self, commit=True):
        ...
        media_files = self.cleaned_data.get("photos", [])
        if commit:
            slug = slugify(property_obj.name)
            photo_order = 0
            video_order = 0
            for file in media_files:
                ctype = getattr(file, "content_type", "")
                if ctype.startswith("image/"):
                    ...
                    compressed_video = compress_video(file, slug, video_order)
